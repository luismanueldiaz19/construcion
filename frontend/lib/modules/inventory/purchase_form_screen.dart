...
class _PurchaseFormScreenState extends State<PurchaseFormScreen> {
  ...
  Future<void> _loadData() async {
    try {
      ...
      final projects = await _apiService.getProyectos();
  ...
  Future<void> _submit() async {
    ...
    if (_selectedProveedorId == null || _selectedProyectoId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Seleccione proveedor y proyecto')));
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    double subtotal = _items.fold(0, (sum, item) => sum + (item['cantidad'] * item['precio_unitario']));
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text('Detalle de Materiales', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              ...
              children: [
                ...
                Expanded(
                  child: DropdownButtonFormField<int>(
                    ...
                    decoration: const InputDecoration(labelText: 'Proyecto Destino', border: OutlineInputBorder()),
                    items: _proyectos.map((p) => DropdownMenuItem<int>(value: p['id'], child: Text(p['nombre']))).toList(),
              ...
              children: [
                Expanded(
                  child: DropdownButtonFormField<String>(
                    value: _tipoCompra,
                    decoration: const InputDecoration(labelText: 'Tipo de Compra', border: OutlineInputBorder()),
                    items: const [
                      DropdownMenuItem(value: 'Contado', child: Text('Contado (Pago Inmediato)')),
                      DropdownMenuItem(value: 'Crédito', child: Text('Crédito (Cuenta por Pagar)')),
                    ],
                ...
                Expanded(
                  child: InkWell(
                    onTap: () async {
                      final date = await showDatePicker(context: context, initialDate: _fecha, firstDate: DateTime(2000), lastDate: DateTime(2100));
                      if (date != null) setState(() => _fecha = date);
                    },
                    child: InputDecorator(
                      decoration: const InputDecoration(labelText: 'Fecha de Factura', border: OutlineInputBorder()),
                      child: Row(
        ...
        children: [
          Expanded(
            ...
            child: DropdownButtonFormField<int>(
              ...
              items: _materiales.map((m) => DropdownMenuItem<int>(value: m['id'], child: Text("${m['nombre']} (${m['unidad']})"))).toList(),
          ...
          Expanded(
            ...
            child: TextFormField(
              ...
              onChanged: (v) => setState(() => item['cantidad'] = double.tryParse(v) ?? 0.0),
            ),
          ),
          const SizedBox(width: 12),
          Expanded(
            ...
            child: TextFormField(
              keyboardType: TextInputType.number,
              decoration: const InputDecoration(labelText: 'Precio Unit.', border: OutlineInputBorder(), prefixText: '\$'),
              onChanged: (v) => setState(() => item['precio_unitario'] = double.tryParse(v) ?? 0.0),
            ),
          ),
  ...
  Widget _buildTotals(double subtotal, double itbis, double total, NumberFormat f) {
    return Align(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            _buildTotalRow('TOTAL GENERAL:', f.format(total), isBold: true, color: Colors.green[800]),
          ],
        ),
      ),
    );
  ...
  Widget _buildTotalRow(String label, String value, {bool isBold = false, Color? color}) {
    return Row(
      ...
      children: [
        ...
        Text(value, style: TextStyle(fontWeight: FontWeight.bold, fontSize: isBold ? 18 : 14, color: color)),
      ],
    );
  }
  ...
  Widget _buildActions() {
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        child: const Text('PROCESAR COMPRA Y REGISTRAR EN CONTABILIDAD', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
      ),
    );
  }
}
