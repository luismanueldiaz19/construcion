...
class _ProjectDetailsScreenState extends State<ProjectDetailsScreen> {
          ...
          children: [
            Wrap(
              ...
              children: [
                ...
                _buildInfoColumn('Estado', _proyecto['estado'], Colors.orange),
              ],
            ),
            if (_proyecto['estado'] == 'Cotización') ...[
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: () async {
                    try {
                      await _apiService.updateProyectoEstado(
                        _proyecto['id'],
                        'Activo',
                      );
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('¡Proyecto Activado!')),
                      );
                      _refresh();
                    } catch (e) {
                      ScaffoldMessenger.of(
                  ...
                  label: const Text('APROBAR COTIZACIÓN Y ACTIVAR PROYECTO'),
  ...
  Widget _buildProfitabilityCard(NumberFormat f) {
    ...
    final costoReal =
        double.tryParse(_proyecto['costo_real']?.toString() ?? '0') ?? 0;
  ...
  Widget _buildPartidaCard(Map<String, dynamic> partida, NumberFormat f) {
    ...
    return Card(
      ...
      child: ExpansionTile(
        ...
        children: subpartidas.map((s) => _buildSubpartidaTile(s, f)).toList(),
      ),
    );
  }
